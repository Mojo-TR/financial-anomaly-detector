    ...
    for ticker in tickers:
        ...
        with engine.connect() as conn:
            latest_date = conn.execute(
                text("SELECT MAX(date) FROM features WHERE ticker = :ticker"),
                {"ticker": ticker}
            ).scalar()

            if latest_date is not None:
                df_out = df_out[df_out['date'] > latest_date]

            if len(df_out) > 0:
                df_out.to_sql('features', engine, if_exists='append', index=False)
                print(f"{ticker}: inserted {len(df_out)} new rows")
            else:
                print(f"{ticker}: no new rows to insert")
